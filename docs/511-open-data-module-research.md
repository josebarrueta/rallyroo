...
This keeps provider request volume proportional to agencies/feeds rather than Family or subscription count. The selected current-quota profile is one combined two-feed cycle every 120 seconds. That is at most 60 real-time requests per hour before positive jitter; a due 24-hour static-GTFS schedule refresh replaces one combined cycle with one request rather than adding requests. The normalized static snapshot supplies both the station catalog and schedule-driven journey choices, and the last-good snapshot remains available when refresh fails. Throttling never causes an immediate retry.
