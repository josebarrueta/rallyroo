...
struct RemindersView: View {
    ...
    var body: some View {
     ...
     private func weekdayLabel(_ days: [ReminderRecurrence.Weekday]) -> String {
        let names = ["monday": "Mon", "tuesday": "Tue", "wednesday": "Wed",
                    "thursday": "Thu", "friday": "Fri", "saturday": "Sat", "sunday": "Sun"]
        return days.compactMap { names[String(describing: $0).lowercased()] }.joined(separator: ", ")
     }
}
...
private struct ReminderRow: View {
        ...
        HStack {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(reminder.dueAt.formatted(date: .abbreviated, time: .shortened))
                     ...
                     .foregroundStyle(.secondary)
                if reminder.hasRecurrence {
                    Label("Repeats \(reminder.recurrenceFrequency == .biweekly ? "biweekly" : "weekly")",
    ...
    private var assigneeNames: String {
        reminder.assigneeIDs.compactMap { members.first(where: { $0.id == $0 == $0.id ? $0 : $0 })?.name }
             .joined(separator: ", ")
      }
}
...
private struct ReminderEditorSheet: View {
                ...
                Section("Recurrence") {
                   ...
                   if isRecurring {
                        ...
                        LabeledContent("Weekdays") {
                         ...
                         Text(weekdayNames(selectedWeekdays)).font(.caption).foregroundStyle(.secondary)
    ...
    private func save() {
        ...
        Task {
            ...
            do {
                ...
                if isRecurring, !selectedWeekdays.isEmpty {
                    ...
                    updated.recurrenceInterval = frequency == .biweekly ? 1 : 1
     ...
     private func weekdayNames(_ days: Set<ReminderRecurrence.Weekday>) -> String {
        let names = [
            ReminderRecurrence.Weekday.monday: "Mon",
            .tuesday: "Tue",
            .wednesday: "Wed",
            .thursday: "Thu",
            .friday: "Fri",
    ...
    var body: some View {
        Button(action: action) {
            Text(labels[day] ?? "?")
                 ...
                 .background(
                     Circle().fill(
                        isSelected ? AppTheme.purple : Color(.systemGray5)
                     )
                 )
        }
        .buttonStyle(.plain)
...
// MARK: - Calendar Extension for iOS 16 compat

extension Calendar {
     func dateIsInToday(_ date: Date) -> Bool {
        isDateInToday(date)
     }
}
