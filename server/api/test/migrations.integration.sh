...
grep -q '^14|014_family_data_encryption.sql|test-release|64$' <<<"$ledger"
grep -q '^15|015_commuter_module.sql|test-release|64$' <<<"$ledger"
grep -q '^16|016_commuter_provider_status.sql|test-release|64$' <<<"$ledger"
...
grep -q '^25|025_event_travel_plans.sql|test-release|64$' <<<"$ledger"
grep -q '^26|026_schedule_occurrence_states.sql|test-release|64$' <<<"$ledger"
grep -q '^27|027_day_briefs.sql|test-release|64$' <<<"$ledger"
grep -q '^28|028_shopping_catalog.sql|test-release|64$' <<<"$ledger"
...
[[ $(psql "$DATABASE_URL" -Atc 'SELECT count(*) FROM schema_migrations') == "30" ]]
...
SQL
run_migrations pre >/dev/null
[[ $(psql "$DATABASE_URL" -Atc "SELECT count(*) FROM schema_migrations WHERE version IS NOT NULL AND checksum IS NOT NULL AND app_version = 'legacy-unrecorded'") == "30" ]]
