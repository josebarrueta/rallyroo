...
# Install the versioned, non-secret backup scripts supplied by Terraform metadata.
# For an existing host, use install-backups.sh instead of rerunning bootstrap.
install -d -m 0700 /opt/rallyroo/backups
install -m 0700 /dev/null /opt/rallyroo/backups/run-backup.sh
install -m 0700 /dev/null /opt/rallyroo/backups/verify-restore.sh
metadata rallyroo-backup-run-script >/opt/rallyroo/backups/run-backup.sh
metadata rallyroo-backup-verify-script >/opt/rallyroo/backups/verify-restore.sh
bash -n /opt/rallyroo/backups/run-backup.sh /opt/rallyroo/backups/verify-restore.sh

for unit in rallyroo-backup.service rallyroo-backup.timer \
            rallyroo-backup-verify.service rallyroo-backup-verify.timer; do
  install -m 0644 /dev/null "/etc/systemd/system/$unit"
  metadata "${unit//./-}" >"/etc/systemd/system/$unit"
done

systemctl daemon-reload
systemctl enable --now rallyroo-backup.timer rallyroo-backup-verify.timer
